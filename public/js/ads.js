console.log("LaggedAPI Revenue Share: v1.7.5");
var adBreak=function(){};"remove"in Element.prototype||(Element.prototype.remove=function(){this.parentNode&&this.parentNode.removeChild(this)});

//
// to do new2 for v3/lagged.js -> Replace all ajax v3/ calls to v4/
//

LaggedAPI={};!function(){

  function S(){}
function e(e){E(e)}function n(){l()}function r(){l()}function s(){l()}function _(){

//
// to do new2 for v3/lagged.js -> handle all login in parent
//
...
try{
  ...
  var parent_uid=window.parent.userid_ds;
  if(parent_uid>0){
    parent_campaign_id=0;
  }
  if(window.parent.allFireBaseLoaded){
    sendValuetoFB=true;
  }
}catch(e){
  console.log(e);
...
if(currentUrl!='lagged.com'){
	useEvent=true;isOnLagged=false;
}
if(!currentUrl){
  currentUrl="lagged.com";
}

//var approvedDomainsList=['lagged.com'];
approvedDomainsList = ['lagged.com','lagged.app','lagged.dev'];
...
if(!isApprovedDomain){
    ...
    if(!isDevMode){
      ...
      //
      // Message "sorry this game is only playable on Lagged.com"
      //
...
if(isDevMode){
//load CSS file for Dev Mode
var CSSlink=document.createElement("link");
CSSlink.href="https://lagged.com/api/rev-share/devmode.css";
CSSlink.rel="stylesheet";
CSSlink.media="screen";
document.getElementsByTagName("head")[0].appendChild(CSSlink);
}else{
  var CSSlink=document.createElement("link");
  CSSlink.href="https://lagged.com/api/rev-share/base.css";
  CSSlink.rel="stylesheet";
  CSSlink.media="screen";
  document.getElementsByTagName("head")[0].appendChild(CSSlink);
}
...
//
// init api, this loads in adsense code as well
//
...
LaggedAPI.init=function(developer_id,dev_adsense_id,usePreloader){
...
if(isOnApprovedWebsite){
  //
  //DEV TO DO: remove !useEvent when other websites added
  //
try{
  var meta = document.createElement('meta');
  meta.name = "google-adsense-platform-account";
  meta.content = "ca-host-pub-1744887146349857";
...
element.setAttribute('data-ad-host', 'ca-host-pub-1744887146349857');
...
try{
  ...
  adConfig({preloadAdBreaks: 'on', onReady: tryPreloader});
  }catch(e){
    console.log(e);
  }
}catch(e){
...
try{
  if(isOnLagged){
    if(window.parent.aFoundValue&&window.parent.aValueRules&&window.parent.aValueRules.length>0){
      aFoundValue=true;
...
var tryPreloader=function(){
  ...
  //           type: 'start',
  //           name: 'start-game',
  //           beforeAd: () => { disableButtons(true);adConvertsion();},
  //           afterAd: () => { enableButtons();if(parent_campaign_id<1&&!isDevMode){adTimer=-10}else{adTimer=0}},
  //         });
  //       }catch(e){
  //         console.log(e);
  //       }
...
var adConvertsion=function(){
  try{
    ...
    if(aFoundValue){
      ...
      window.parent.gtag('event', 'conversion', {'send_to': 'AW-1055364430/cX1uCOOx4LcBEM6qnvcD'});
    }
  }catch(e){
    console.log(e);
  }
  if(sendValuetoFB){
    try{
      window.parent.sendToFireBase('host', window.location.pathname);
    }catch(e){
      console.log(e);
    }
  }
...
//
LaggedAPI.User={get:function(response){
  var returnData={user: {id:0, name: "Example User", avatar: "https://lagged.com/images/avatars/default-avatar.jpg"}};
  if(isOnLagged&&!isDevMode){
    try{
      returnData.user.id=window.parent.userid_ds;
      returnData.user.name=window.parent.username;
      returnData.user.avatar=window.parent.useravatar;
    }catch(e){
      console.log(e);
    }
  }
...
//
vL=[],b=[];
LaggedAPI.Achievements={save:function(e,t){for(var a=0,n=e.length;a<n;a++)-1===vL.indexOf(e[a])&&(vL.push(e[a]),b.push(e[a]));var r,d;0<b.length?(r=b.length,d=t,setTimeout(function(){if(b.length>r)d({success:!0});else{var e={action:"save"};e.publickey=m,e.awards=b,b=[];var t=JSON.stringify(e),a=g.base64(t);
...
if(useEvent){
//fire event with achievement data: a
try{
window.parent.postMessage('awards|'+a,'*');
d({success:!0});
}catch(e){
...
c("award","award","save",a,f,d)}},35)):t({success:!0})},show:function(){
  try{window.parent.openAwards()}catch(e){
...
LaggedAPI.Scores={save:function(e,t){u=e.board;
if(e.score===0||e.score<0){return;}
...
var a={action:"save"};a.publickey=m,a.board=e.board,a.score=e.score;var n=JSON.stringify(a),r=g.base64(n);
...
c("highscore","hs","save",r,C,t)},load:function(e,t){
if(!useEvent){
  try{
...
}};var E,f=function(e,t){var a={success:!0};e&&!0===e.success?!0===e.data.show&&N(e.data.achdata,e.user):(alert("Error: Achievement did not save!"),console.log(e),a.success=!1,a.errormsg="Error: Achievement did not save!"),t&&t(a)},C=function(e,t){
  
  var a={success:!0};e&&!0===e.success?(hsData=e,function(){
    

    //
    // to do new2 : send data to parent
    //
    try{
      window.parent.showHSSaved(hsData);

      //add call for older games
      LaggedAPI.GEvents.next(function(){console.log("ad done")});
    }catch(e){
      console.log(e);
    }
...
//var E=!0;2===e&&(E=!1),isMobile=false;var a=document.createElement("div");a.id="tabsButtonWraps",a.className="logintabs",(n=document.createElement("button")).className="tabs_links active",n.style.width="50%";var n,r=document.createTextNode("Sign Up for Free");n.appendChild(r),a.appendChild(n),(n=document.createElement("button")).className="tabs_links",n.style.width="50%",n.onclick=function(){!function(){Q.getElementById("signupFormWrap").remove();var e=document.createElement("div");
...
// }()):(S(),Q.getElementById("leaderboard-modal").remove(),alert("Error: Could not save high score!"),console.log(e),a.success=!1,a.errormsg="Error: Could not save high score!"),t&&t(a)


}()):(S(),Q.getElementById("leaderboard-modal").remove(),alert("Error: Could not save high score!"),console.log(e),a.success=!1,a.errormsg="Error: Could not save high score!"),t&&t(a)
},y=function(e){},w=0,N=function(awardInfo){
    //
    // to do new2 : send data to parent
    //
    try{
      window.parent.showAwardSaved(awardInfo);
    }catch(e){
      console.log(e);
    }

  
  //h=window.parent.isFullscreen,Q=h?document:window.parent.document,4<++w&&(w=1);var a="achievement_pops_"+w,n=Q.createElement("div");n.id="achievementPopWrap",n.className=a,n.onclick=function(){Q.getElementsByClassName(a)[0].remove()};var r="Achievement Saved";1<e.acount&&(r=e.acount+" Achievements Saved");var d=document.createElement("div");d.className="achievement_title";var i=document.createTextNode(r);d.appendChild(i),n.appendChild(d);var o=document.createElement("div");o.className="achievement_desc";var l=document.createTextNode(e.name);o.appendChild(l),n.appendChild(o);var s=document.createElement("div");s.className="achievement_xp";var c=document.createTextNode("+"+e.points+"xp");if(s.appendChild(c),n.appendChild(s),Q.body.appendChild(n),t)try{window.parent.newLevel(t)}catch(e){console.log(e)}setTimeout(function(){Q.getElementsByClassName(a)[0]&&(p(Q.getElementsByClassName(a)[0]),w--,setTimeout(function(){Q.getElementsByClassName(a)[0]&&Q.getElementsByClassName(a)[0].remove()},200))},4e3)
...
LaggedAPI.GEvents={start:function(adResponse){
  console.log("event START");

  try{
    startGameManually=setTimeout(function(){
      //
      // to do new2 v3 -> wrap in try
      //
      try{
        adResponse();
      }catch(e){
  ...
  adBreak({
    ...
    name: 'start-game',
    beforeAd: () => { disableButtons(true);adConvertsion();},
    afterAd: () => { enableButtons();if(parent_campaign_id<1&&!isDevMode){adTimer=-15}else{adTimer=0}},
    adBreakDone: () => {
      try{
        adResponse();
      }catch(e){
  ...
  //do not check again if reward is ready
  if(canGiveReward){
    if(isDevMode){
      console.log("Do not check reward ad again until previous reward ad is viewed");
    }
    canShowButton(true,keepShowAdFn);
    return
  }
...
adBreak({
  type: 'reward',
  name: 'reward',
  beforeAd: () => { disableButtons(false); },
  afterAd: () => { enableButtons(false);if(parent_campaign_id<1&&!isDevMode){adTimer=30}else{adTimer=60}},
  beforeReward: (showAdFn) => { beforeReward(showAdFn,canShowButton); },
  adDismissed: () => {adDismissed(callback);canGiveReward=false;keepShowAdFn=function(){}},
  adViewed: () => { adCompled(callback);canGiveReward=false;keepShowAdFn=function(){}},
});
...
var adCompled=function(callback){
  ...
  if(sendValuetoFB){
    try{
      window.parent.sendToFireBase('host_reward', window.location.pathname);
    }catch(e){
      console.log(e);
    }
  }
...
try{
  //
  //to do new2: v3 -> remove anything except mobilerightnew
  //
...
LaggedAPI.APIAds={show:function(response){
...
try{
//to do -> pass along rev share data?
window.parent.postMessage('apiAds','*');
}catch(e){
  console.log(e);
}
//auto success ad after 10 seconds
try{
setTimeout(function(){
  try{
    response();
...
//
// listen for parent variables on non lagged
//
...
var A=function(){},T=function(e){var t=document.createDocumentFragment(),a=document.createElement("div");for(a.innerHTML=e;a.firstChild;)t.appendChild(a.firstChild);return t};
Element.prototype.remove=function(){this.parentElement.removeChild(this)},NodeList.prototype.remove=HTMLCollection.prototype.remove=function(){for(var e=this.length-1;0<=e;e--)this[e]&&this[e].parentElement&&this[e].parentElement.removeChild(this[e])}}();
